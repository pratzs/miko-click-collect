...
// Per-listing allowlist for the IMAGE value-claim-number rule. That rule exists to
// stop "82 hrs saved" style marketing statistics being baked into a banner. It
// cannot tell those apart from the app's OWN UI doing its job, and this app's UI is
// full of legitimate durations: a merchant-set preparation time ("Usually ready in
// 2 hours") and store opening hours are exactly the numbers the gate's own comment
// says are fine. Each allowed string is exact, needs a reason, and is printed on
// every run so a real marketing statistic can never hide behind this.
let ALLOWED_IMAGE_NUMBERS = {};
...
function scan(label, text, opts = {}) {
  ...
  if (opts.isImage) {
    ...
    if (/\d+\s*(%|hrs?|hours|x faster|times faster)/i.test(text)) {
      const reason = ALLOWED_IMAGE_NUMBERS[text.trim()];
      if (typeof reason === "string" && reason.trim().length > 0) {
        ALLOWANCES_USED.push(`${label}: "${text.trim()}" - ${reason}`);
      } else {
...
if (require.main === module) {
  ...
  ALLOWED_IMAGE_NUMBERS = listing.allowedImageNumbers || {};
  let failures = 0;
  let warns = 0;
